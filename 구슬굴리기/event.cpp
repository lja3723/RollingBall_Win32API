...
{
	startPos[button_idx] = { posX, posY };
	prevPos[button_idx] = { posX, posY };
}
...
void RollingBall::MouseEvent::_staticState::_drag::trace_mouseMove()
{
	for (int i = 0; i < numofButtons; i++)
		if (isDragging[i])
		{
		...
		else if (isPosValid(startPos[i]) && isPosValid(prevPos[i]))
		{
			isDragging[i] = TRUE;
		}
}
